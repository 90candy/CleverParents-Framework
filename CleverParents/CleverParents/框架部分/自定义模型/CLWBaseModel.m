...
@implementation CLWBaseModel

+ (LKDBHelper *)getUsingLKDBHelper {
    static LKDBHelper *db;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *sqlitePath = [CLWBaseModel downloadPath];
        //数据库文件路径
        NSString *dbpath = [sqlitePath stringByAppendingPathComponent:[NSString stringWithFormat:@"CleverDB.db"]];
...
+ (NSString *)downloadPath {
    ...
    //文件夹名字
    NSString *downloadPath = [documentPath stringByAppendingPathComponent:@"CleverDB"];
    NSLog(@"\n数据库文件夹：\n%@",downloadPath);
    return downloadPath;
}
@end
